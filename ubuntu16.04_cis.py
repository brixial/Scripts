...
def step1():
    ...
    with open("ubuntu16.txt", "a+" , buffering=1) as step1:
        ...
        with subprocess.Popen(["modprobe" , "-v" , "-r" , "freevxfs"], stdout=step1 ) as freevxfs:
            stdout,stderr = freevxfs.communicate()
        step1.write ("\n# Ensure mounting of jffs2 filesystems is disabled\n")
        with subprocess.Popen(["modprobe" , "-v" , "-r" , "jffs2"], stdout=step1 ) as jffs2:
            stdout,stderr = jffs2.communicate()
        step1.write ("\n# Ensure mounting of hfs filesystems is disabled\n")
        with subprocess.Popen(["modprobe" , "-v" , "-r" , "hfs"], stdout=step1 ) as hfs:
            stdout,stderr = hfs.communicate()
        step1.write ("\n# Ensure mounting of hfsplus filesystems is disabled\n")
        with subprocess.Popen(["modprobe" , "-v" , "-r" , "hfsplus"], stdout=step1 ) as hfsplus:
            stdout,stderr = hfsplus.communicate()
        ...
        cmd = cmd.split()
        cmd2 = "chmod og-rwx /boot/grub/grub.cfg"
        cmd2 = cmd2.split()
        ...
        #with subprocess.Popen(["systemctl" , "daemon-reload"], stdout = step1) as tmp17:
         ...
         step1.write("Ensure address space layout randomization (ASLR) is enabled \n")
        with open ("/etc/sysctl.conf" , "rt" , buffering = 1 ) as tmp:
            result_tmp = tmp.read()
            tmp.close()
        with open ("/etc/sysctl.conf" , "wt" , buffering = 1 ) as tmp:
        ...
        step1.write("ensure prelink is disabled\n")
        os.system("prelink -ua")
        os.system("apt-get remove prelink -y")
        step1.write("# Ensure SELinux is installed\n")
        with open("/etc/default/grub" , "rt" , buffering = 1) as tmp:
            result_tmp = tmp.read()
            result_tmp = result_tmp.rstrip()
            result_tmp = re.sub('GRUB_CMDLINE_LINUX_DEFAULT=*' , 'GRUB_CMDLINE_LINUX_DEFAULT="quiet"' , result_tmp )
            tmp.close()
        ...
        with open("/etc/selinux/config" , "rt" , buffering = 1) as tmp:
            result_tmp = tmp.read()
            result_tmp = result_tmp.rstrip()
            result_tmp = re.sub('SELINUX=*' , 'SELINUX=enforcing' , result_tmp )
            tmp.close()
        with open("/etc/selinux/config" , "wt" , buffering = 1) as tmp:
            tmp.write(result_tmp)
            tmp.close()
        ...
        with open("/etc/selinux/config" , "rt" , buffering = 1) as tmp:
            result_tmp = tmp.read()
            result_tmp = result_tmp.rstrip()
            result_tmp = re.sub('SELINUXTYPE=*' , 'SELINUXTYPE=ubuntu' , result_tmp )
            tmp.close()
        with open("/etc/selinux/config" , "wt" , buffering = 1) as tmp:
            tmp.write(result_tmp)
            tmp.close()
        step1.write("Ensure all AppArmor Profiles are enforcing")
        os.system("aa-enforce /etc/apparmor.d/*")
        step1.write("Ensure SELinux or AppArmor are installed")
        os.system("apt-get install selinux -y")
        os.system("apt-get install apparmor -y")
        ...
        with open("/etc/dconf/profile/gdm" , "a+" , buffering = 1)as tmp:
            tmp.write("user-db:user\n")
            tmp.write("system-db:gdm\n")
            tmp.write("file-db:/usr/share/gdm/greeter-dconf-defaults")
            tmp.close()
        with open("/etc/dconf/db/gdm.d/01-banner-message" , "a+" , buffering = 1)as tmp:
...
def step2():
    with open("ubuntu16.txt", "a+" , buffering=1) as step2:
        step2.write("Begin chapter 2")
        step2.write("# Ensure xinetd is not installed\n")
        cmd = "apt-get remove -y xinetd"
        cmd = cmd.split()
        with subprocess.Popen(cmd, stdout = step2 ) as tmp1:
            stdout,stderr = tmp1.communicate()
        step2.write("# Ensure openbsd-inetd is not installed\n")
        cmd = "apt-get remove -y openbsd-inetd"
        cmd = cmd.split()
        with subprocess.Popen(cmd, stdout = step2 ) as tmp01:
            stdout,stderr = tmp01.communicate()
        step2.write("# Ensure time synchronization is in use\n")
        cmd = "apt-get install -y chrony"
        cmd = cmd.split()
        cmd2 = "apt-get install -y ntp"
        cmd2 = cmd2.split()
        with subprocess.Popen(cmd , stdout = step2) as tmp2:
            stdout,stderr = tmp2.communicate()
        with subprocess.Popen(cmd2 , stdout = step2) as tmp02:
            stdout,stderr = tmp02.communicate()
        step2.write("Ensure ntp is Configured")
        with open("/etc/ntp.conf" , "a+" , buffering=1) as ntp01:
            ntp01.write("restrict -4 default kod nomodify notrap nopeer noquery")
            ntp01.write("restrict -6 default kod nomodify notrap nopeer noquery")
            ntp01.write("server <remote-server>")
            ntp01.close()
        with open(" /etc/init.d/ntp" , "a+" , buffering=1) as ntp01:
            ntp01.write("RUNASUSER=ntp")
            ntp01.close()
        ...
        step2.flush()
        step2.write("# Ensure rsync service is not enabled\n")
        cmd = "systemctl disable rsync"
        cmd = cmd.split()
        ...
        cmd = cmd.split()
        cmd2 = "systemctl disable isc-dhcp-server6"
        cmd2 = cmd2.split()
        with subprocess.Popen(cmd, stdout = step2) as tmp19:
            stdout,stderr = tmp19.communicate()
        with subprocess.Popen(cmd2, stdout = step2) as tmp019:
            stdout,stderr = tmp019.communicate()
        ...
        cmd = "apt-get -y remove ldap-utils"
        cmd = cmd.split()
        with subprocess.Popen(cmd, stdout = step2) as tmp24:
            stdout,stderr = tmp24.communicate()
        step2.write("# Ensure X Window System is not installed\n") 
        cmd = "apt-get remove -y xserver-xorg*"
        cmd = cmd.split()
        with subprocess.Popen(cmd, stdout = step2) as tmp25:
            stdout,stderr = tmp25.communicate()
        step2.write("# Ensure rsh client is not installed \n") 
        cmd = "apt-get remove -y rsh-client rsh-redone-client"
        cmd = cmd.split()
        with subprocess.Popen(cmd, stdout = step2) as tmp26:
            stdout,stderr = tmp26.communicate()
        step2.write("# Ensure talk client is not installed \n") 
        cmd = "apt-get remove -y talk"
        cmd = cmd.split()
        with subprocess.Popen(cmd, stdout = step2) as tmp26:
            stdout,stderr = tmp26.communicate()
        step2.write("# Ensure mail transfer agent is configured for local-only mode\n")
        with open("/etc/postfix/main.cf" , "rt" , buffering = 1) as tmp:
            result_tmp = tmp.read()
            result_tmp = result_tmp.rstrip()
            result_tmp = re.sub('inet_interfaces = *' , 'inet_interfaces = loopback-only' , result_tmp )
            tmp.close()
        with open("/etc/postfix/main.cf" , "wt" , buffering = 1) as tmp:
            tmp.write(result_tmp)
            tmp.close()
...
def step3():
    with open("ubuntu16.txt", "a+" , buffering = 1) as step3:
        ...
        with open ("/etc/sysctl.conf" , "a" , buffering = 1) as kernel7:
            ...
            kernel7.write("net.ipv4.conf.all.rp_filter = 1")
        with subprocess.Popen(['sysctl' , '-w' , 'net.ipv4.conf.default.rp_filter=1'] , stdout = step3) as tmp28:
            stdout,stderr = tmp28.communicate()
        with subprocess.Popen(['sysctl' , '-w' , 'net.ipv4.conf.all.rp_filter=1'] , stdout = step3) as tmp028:
            stdout,stderr = tmp028.communicate()
        ...
        step3.write("Ensure TCP Wrappers is installed")
        os.system("apt-get install -y tcpd")
        step3.write("Ensure /etc/hosts.deny is configured")
        os.system("echo "ALL: ALL" >> /etc/hosts.deny")
        step3.write("Ensure permissions on /etc/hosts.allow are configured")
        os.system("chown root:root /etc/hosts.allow")
        os.system("chmod 644 /etc/hosts.allow")
        step3.write("Ensure permissions on /etc/hosts.deny are configured")
        os.system("chown root:root /etc/hosts.deny")
        os.system("chmod 644 /etc/hosts.deny")
        ...
        step3.write("# Ensure iptables is installed")
        os.system("apt-get install -y iptables")
        ...
        step3.write("Ensure default deny firewall policy")
        os.system(" iptables -P INPUT DROP")
        os.system(" iptables -P OUTPUT DROP")
        os.system(" iptables -P FORWARD DROP")

        step3.write("Ensure loopback traffic is configured")
        os.system("iptables -A INPUT -i lo -j ACCEPT")
        os.system("iptables -A OUTPUT -o lo -j ACCEPT")
        os.system("iptables -A INPUT -s 127.0.0.0/8 -j DROP")
        step3.write("#  Ensure wireless interfaces are disabled\n")
        step3.close()

def step4():
    with open("ubuntu16.txt", "a+" , buffering=1) as step4:
        ...
        with subprocess.Popen(['systemctl' , 'enable' , 'auditd'] , stdout = step4) as tmp2:
            stdout,stderr = tmp2.communicate()
        ...
        with open("/etc/audit/rules.d/audit.rules" , "wt" , buffering = 1) as tmp:
            ...
            tmp.write ("-w /var/log/tallylog -p wa -k logins\n")
            tmp.close()
        ...
        step4.write("Configure Syslog-ng")
        os.system("apt-get install -y syslog-ng")
        os.system("update-rc.d syslog-ng enable")

        step4.write("# Ensure permissions on all logfiles are configured\n")
        os.system("chmod -R g-wx,o-rwx /var/log/*")
...
def step5():
    with open("ubuntu16.txt", "a+" , buffering=1) as step5:
        step5.write("# Ensure cron daemon is enabled\n")
        cmd = "systemctl enable crond"
        cmd = cmd.split()
        ...
        step5.write("#  Ensure SSH Protocol is set to 2\n")
        ...
        with open ("/etc/ssh/sshd_config" , "wt" , buffering = 1 ) as sshd:
            result_tmp = re.sub(".*Protocol*" , "Protocol 2" , result_tmp)
            sshd.write(result_tmp)
            sshd.close()



        step5.write("# Ensure SSH LogLevel to INFO\n")
        with open ("/etc/ssh/sshd_config" , "rt" , buffering = 1 ) as sshd:
            result_tmp = sshd.read()
            sshd.close()      
        with open ("/etc/ssh/sshd_config" , "wt" , buffering = 1 ) as sshd:
            result_tmp = re.sub(".*LogLevel.*" , "LogLevel INFO" , result_tmp)
            sshd.write(result_tmp)
            sshd.close()
        ...
        os.system("apt-get install libpam-pwquality")
     ...
     #       result_tmp = pwquality.read()
      #      pwquality = pwquality.close()
       # with open ("/etc/authselect/custom/custom-profile/system-auth" , "wt" , buffering = 1 ) as pwquality:
        ...
        with open ("/etc/pam.d/common-auth" , "rt" , buffering = 1 ) as pwquality:
            result_tmp = pwquality.read()
            pwquality = pwquality.close()
        with open ("/etc/pam.d/common-auth" , "wt" , buffering = 1 ) as pwquality:
            pwquality.write(result_tmp)
        ...
        with open ("/etc/pam.d/common-password" , "rt" , buffering = 1 ) as pwquality:
            result_tmp = pwquality.read()
            pwquality = pwquality.close()
        with open ("/etc/pam.d/common-password" , "wt" , buffering = 1 ) as pwquality:
            result_tmp = re.sub(".*password.*required.*pam_pwhistory.so.*" , "\npassword required pam_pwhistory.so remember=5\n" , result_tmp)
            pwquality.write(result_tmp)
            pwquality.close()
        ...
        with open ("/etc/authselect/custom/custom-profile/password-auth" , "rt" , buffering = 1 ) as pwquality:
            ...
            pwquality = pwquality.close()
        with open ("/etc/authselect/custom/custom-profile/password-auth" , "wt" , buffering = 1 ) as pwquality:
            result_tmp = re.sub(".*password.*[success=1 default=ignore].*pam_unix.so.*" , "\npassword [success=1 default=ignore] pam_unix.so sha512\n" , result_tmp)
            pwquality.write(result_tmp)
            pwquality.close()
...
def step6():
     with open("ubuntu16.txt", "a+" , buffering=1) as step6:
